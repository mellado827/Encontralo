...
function EditReport(props) {
  ...
  const nombreUsuarioNuevo = report.nombreUsuario
    ? report.nombreUsuario
    : caso.nombreUsuario;
  const descripcionUsuarioNuevo = report.descripcionUsuario
    ? report.descripcionUsuario
    : caso.descripcionUsuario;
  ...
  const ViralInfo = () => {
          ...
          const hembra = `${
            tipomascota.substr(0, tipomascota.length - 1) + "a"
                    ...
                    ${
                      estadoNuevo
                        .toLowerCase()
                        .substr(0, estadoNuevo.length - 1) + "a"
                    }`;
          return hembra;
        default:
          break;
    ...
    const viralInfo = `${realSex()} en ${departamentoNuevo}, ${localidadNuevo}, más específicamente en: ${lugarNuevo} ${
      selectedDate ? `el día ${fecha}` : ``
    }
        ${horaNuevo ? `a las ${horaNuevo}.` : ``}
        ${
          nombreNuevo
            ? `Responde al nombre de ${nombreNuevo}`
            : `Se desconoce el nombre`
        }, ${razaNuevo ? `raza ${razaNuevo}` : `raza no especificada`}.
        ${chip()}. Datos de vital importancia: ${descripcionNuevo}. ${
      nombreUsuarioNuevo
        ? `La persona responsable es ${nombreUsuarioNuevo}.`
        : ``
    }
        ${
          descripcionUsuarioNuevo
            ? `Datos adicionales de la persona responsable: ${descripcionUsuarioNuevo}`
            : ``
        }
  ...
  const editReport = (e) => {
    ...
    let ok =
      ...
      !descripcionUsuario.length &&
      (!caso.imagen || !imagenNueva);
    ...
    if (ok === true) {
      ...
      try {
        Swal.fire({
          ...
          text:
            "La información modificada quedaría en el sitio, no al difundirse el reporte. Si llegás a realizar un reporte troll, tu cuenta será eliminada.",
          icon: "warning",
          ...
          if (result.isConfirmed) {
            try {
              ...
              const editReport = await axiosClient
                .patch(`/reportes/${idcaso}`, formData, {
                  headers: {
            ...
            <div className="report_container">
              ...
              <div className="owner_form m-3 d-flex flex-column">
                ...
                <p className="text_fontstyle mt-4 text-center grey_color">
                  En caso de que los datos que aparecen en pantalla no
                  correspondan con su usuario, modifíquelos antes de reportar la
                  desaparición.
                </p>
                <Link
                  to="/datospersonales"
